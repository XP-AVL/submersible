...
namespace Encounter.Runtime.Creatures
{
    ...
    public class JellyfishGill : MonoBehaviour
    {
        ...
        private JellyfishBell _bellReference;
    
        // Each gill is a LineRenderer component that draws a curved line
        private List<LineRenderer> _gillLines;
        ...
        private List<int[]> _gillColumnIndices;
        ...
        private void Start()
        {
            ...
            _bellReference = GetComponent<JellyfishBell>();
        
            if (_bellReference == null)
            {
        ...
        private void InitializeGillSystem()
        {
            ...
            _gillLines = new List<LineRenderer>();
            _gillColumnIndices = new List<int[]>();
        ...
        private List<int> SelectGillPositions()
        {
            ...
            for (int r = 0; r < _bellReference.radialSegments; r += gillSpacing)
            {
                positions.Add(r);
            }
        ...
        private void CreateSingleGill(int radialIndex, int gillIndex)
        {
            ...
            _gillLines.Add(lineRenderer);
            _gillColumnIndices.Add(columnIndices);
        }

        private void SetupLineRendererProperties(LineRenderer lineRenderer)
        {
        ...
        private int[] CalculateColumnIndices(int radialIndex)
        {
            ...
            int[] indices = new int[_bellReference.heightSegments + 1];
        
            for (int h = 0; h <= _bellReference.heightSegments; h++)
            {
                indices[h] = _bellReference.GetVertexIndex(h, radialIndex);
            }
        ...
        private void UpdateGillPositions()
        {
            if (_gillLines == null || _bellReference == null) return;
        
            // Update each gill line
            for (int gillIndex = 0; gillIndex < _gillLines.Count; gillIndex++)
            {
                UpdateSingleGill(gillIndex);
            }
        }

        private void UpdateSingleGill(int gillIndex)
        {
            LineRenderer lineRenderer = _gillLines[gillIndex];
            int[] columnIndices = _gillColumnIndices[gillIndex];
            ...
            for (int pointIndex = 0; pointIndex < columnIndices.Length; pointIndex++)
            {
                ...
                Vector3 bellPosition = _bellReference.GetVertexPosition(columnIndices[pointIndex]);
        ...
        private void CleanupGills()
        {
            if (_gillLines != null)
            {
                // Destroy each gill GameObject
                foreach (LineRenderer lr in _gillLines)
                {
        ...
        private void OnValidate()
        {
            ...
            if (Application.isPlaying && _gillLines != null)
            {
                RegenerateGills();
            }
        }
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (_bellReference == null) return;
        
            // Draw small spheres at gill attachment points
            Gizmos.color = Color.cyan;
            for (int r = 0; r < _bellReference.radialSegments; r += gillSpacing)
            {
                Vector3 bottomPos = _bellReference.GetVertexPosition(
                    _bellReference.GetVertexIndex(_bellReference.heightSegments, r)
                );
