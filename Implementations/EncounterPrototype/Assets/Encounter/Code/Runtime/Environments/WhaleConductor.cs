...
public class WhaleConductor : MonoBehaviour
{
    ...
    [SerializeField] private float[] targetIntervals = { 0, 4, 7 };  // Major chord! (0 = base, 4 = major third, 7 = perfect fifth)
    [SerializeField] private float intervalTolerance = 1.5f;         // How many semitones off we allow (1.5 = pretty forgiving!)
    [SerializeField] private float noteHoldTime = 0.3f;              // How long they need to hold each note
...
=======
        ...
        if (useMelodicLock)
        {
            ...
            Debug.Log($"   (That's like singing 'Do-Mi-Sol' - a major chord!)");
        }
    }
...
=======
    ...
    void HandleMelodicDetection()
    {
        ...
        // Step 1: Get the current pitch using our enhanced method!
        float currentPitch = EstimateEnhancedPitch(analysisBuffer);
    ...
    void ProcessMelodicNote(float pitch)
    {
        if (basePitch == 0)
        {
            ...
            basePitch = pitch;
            detectedIntervals.Add(0); // First interval is always 0 (unison with itself!)
            Debug.Log($"🎵 BASE NOTE established: {pitch:F1}Hz (let's call this 'Do')");
            return;
        }
        ...
        {
            detectedIntervals.Add(semitones);
            Debug.Log($"✅ CORRECT! Note {detectedIntervals.Count} of {targetIntervals.Length}");
            
            // Check if melody is complete!
            if (detectedIntervals.Count == targetIntervals.Length)
>>>>>>> Stashed changes
            {
                Debug.Log("🎊 MELODY COMPLETE! TRIGGERING WHALE CHORUS! 🎊");
                TriggerWhaleChorus();
                ResetMelodicDetection();
            }
        }
...
=======
    ...
    void TriggerWhaleChorus()
    {
        ...
        // Pass the sung melody pitches to the chorus!
        float[] melodyPitches = new float[detectedIntervals.Count];
        melodyPitches[0] = basePitch; // Start with the base pitch
    ...
    System.Collections.IEnumerator ConductMelodicChorus(float[] melodyPitches)
    {
        chorusActive = true;
...
>>>>>>> Stashed changes
        ...
        if (useMelodicLock && melodyPitches.Length > 0)
        {
            ...
            {
                Debug.Log($"🎵 Whales singing note {noteIndex + 1}: {melodyPitches[noteIndex]:F1}Hz");
            ...
            Debug.Log("🎊 FINALE: All whales together! 🎊");
...
=======
        ...
        Debug.Log("Whale melodic chorus complete! The ocean remembers your song!");
    }
...
=======
        ...
        GUI.Label(new Rect(20, 35, 320, 20), $"Whales found: {allWhales?.Length ?? 0}");
        GUI.Label(new Rect(20, 55, 320, 20), $"Microphone: {(isListening ? "Active" : "Inactive")}");
        GUI.Label(new Rect(20, 75, 320, 20), $"Mode: {(useMelodicLock ? "🎵 Melodic Lock" : "🎤 Sustained Note")}");
        ...
        if (useMelodicLock)
        {
            ...
            for (int i = 0; i < targetIntervals.Length; i++)
            {
                ...
                GUI.color = boxColor;
                GUI.Box(new Rect(startX + i * (boxWidth + 10), startY + yOffset, boxWidth, boxHeight), 
                       i == 0 ? "Base Note" : $"+{targetIntervals[i]} semitones");
                GUI.color = Color.white;
            }
            ...
            {
                GUI.Label(new Rect(20, 160, 320, 20), 
                         $"Sing the next note! ({targetIntervals.Length - detectedIntervals.Count} to go)");
            }
        }
        else
        {
